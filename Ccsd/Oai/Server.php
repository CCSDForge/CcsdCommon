        ...
        foreach ( $this->getIdentity($url) as $config=>$value ) {
            if ( is_array($value) ) {
                foreach ( $value as $node=>$val ) {
                    ...
                    if ( is_array($val) && isset($val['attributes']) && is_array($val['attributes']) && isset($val['nodes']) && is_array($val['nodes']) ) {
                        $desc = $this->_xml->createElement($node);
                        foreach ( $val['attributes'] as $ns=>$attribute ) {
                            $desc->setAttribute($ns, $attribute);
                        }
                        foreach ( $val['nodes'] as $entry=>$node ) {
                            if ( is_array($node) ) {
                                ...
                                $desc->appendChild($this->_xml->createElement($entry, $node));
                            }
                        }
        ...
        if ( is_array($docids) && count($docids) ) {
            try {
                ...
                foreach ( $docids as $document ) {
                    if ( is_array($document) && isset($document['header']) && $document['header'] && isset($document['metadata']) && $document['metadata'] ) {
                        $record = $this->_xml->createElement('record');
