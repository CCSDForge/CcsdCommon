            ...
            if ($this->_isLogged) {
                ...
                <form class="form-inline pull-right"
                    ...
                    <input type="hidden" name="forward-controller"
                           value="<?php echo Zend_Controller_Front::getInstance()->getRequest()->getControllerName();?>" />
                    <input type="hidden" name="forward-action"
                           value="<?php echo Zend_Controller_Front::getInstance()->getRequest()->getActionName();?>" />
                    <?php
                $forwardParams = Zend_Controller_Front::getInstance()->getRequest()->getParams();
                ...
                ?>
					...
					<li><a href="#" onclick="$('#form-login').submit();"><?php echo $view->translate('Connexion');?></a></li>
                        <?php
                        if ($this->_active == self::APP_HAL) {
                            ...
                            ?><li><a id="getorcid" href="https://orcid.org/oauth/authorize?client_id=APP-O6Y5HZD2SFM7ON6Z&response_type=code&scope=/authenticate&redirect_uri=<?php echo HAL_URL; ?>/user/coext?url=<?php echo $urlredirect;?>"><?php echo $this->view->translate('Connexion avec ORCID');?></a></li>
						<?php }?>
                        <li class="divider"></li>
