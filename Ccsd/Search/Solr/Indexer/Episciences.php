...
class Ccsd_Search_Solr_Indexer_Episciences extends Ccsd_Search_Solr_Indexer
{
    ...
    protected function addMetadataToDoc($docId, $ndx)
    {
        ...
        */
        ...
        $paperData = self::getDocidData($docId);
        $paperVolumesData = self::getPaperVolumesData($docId);
        ...
        $volumeTranslations = (array_key_exists('volumes', $review['TRANSLATIONS'])) ? $review['TRANSLATIONS']['volumes'] : null;
        ...
        $review_title = self::cleanChars($review['NAME']);
        $revue_date_creation = date_format(new DateTime($review['CREATION']), "Y-m-d\Th:i:s\Z");
        $es_doc_url = 'http://' . $review['CODE'] . '/' . $docId;
        $es_pdf_url = $es_doc_url . '/pdf';

        $citation = $review['CODE'] . ':' . $docId . ' - ' . $review['NAME'] . ', ';

        $oDate = new Zend_Date();
        if (strtotime($paperData['PUBLICATION_DATE'])) {
            $citation .= $oDate->set(strtotime($paperData['PUBLICATION_DATE']))->toString(Zend_Date::DATE_LONG, 'en');
        }

        if (array_key_exists('volumes', $review['TRANSLATIONS']) && $paperData['VID'] && array_key_exists('volume_' . $paperData['VID'] . '_title', $review['TRANSLATIONS']['volumes']['en'])) {
            $citation .= ', ' . $review['TRANSLATIONS']['volumes']['en']['volume_' . $paperData['VID'] . '_title'];
        }
        ...
        $dataToIndex = [
            ...
            'revue_issn_s' => (array_key_exists('ISSN', $review['SETTINGS'])) ? $review['SETTINGS']['ISSN'] : null,
        ...
        // 'author_id_i' => 0,
        // 'author_firstname_s' => $lastname,
        // 'author_lastname_s' => $lastname,
        // 'author_othername_s' => $lastname,

        /*
         * 'lastname_s' => $lastname, 'firstname_s' => $firstname,
         * 'middlename_s' => self::cleanChars($row['MIDDLENAME']), 'fullname_s'
         * => $lastname . ', ' . $firstname, 'email_s' => $row['EMAIL'], 'url_s'
         * => $row['URL'], 'research_team_t' =>
         * self::cleanChars($row['RESEARCHTEAM']), 'organism_t' =>
         * self::cleanChars($row['ORGANISM'])
         */
        ...
        // section data
        if ($paperData['SID']) {
            $section = $this->getSection($paperData['SID']);
    ...
    private static function cleanAuthorName($name)
    {
