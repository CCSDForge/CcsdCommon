...
abstract class Ccsd_Website_Navigation 
{
     ...
     * @param reload : Regénère la liste des types de pages
     * @return array
     */
    ...
    public function addPage($page)
    {
    	if (is_string($page)) {
    		...
    		$page = new $class($this, array('languages' => $this->_languages));
    	}
     ...
     * @param int $pageIdx index de la page à supprimer
     */
    public function deletePage($pageIdx)
    {
        //Suppression de la page
    	unset($this->_pages[(int) $pageIdx]);
    	
    	//Supression de la page dans le tableau d'ordre des pages
    	foreach ($this->_order as $pageid => $spages) {
    		if ($pageid == $pageIdx) {
    			unset($this->_order[$pageid]);break;	
    		}
    		if (count($spages) > 0) {
    			foreach ($spages as $i => $spageid) {
    				if ($spageid == $pageIdx) {
    					unset($this->_order[$pageid][$i]);break;
    				}
    			}
    		}
    	}    	
